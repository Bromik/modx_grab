...
Ext.override(miniShop2.panel.Product, {
	...
	msoptionscolorOriginals: {
		getFields: miniShop2.panel.Product.prototype.getFields,
	},

	getFields: function (config) {
		var fields = this.msoptionscolorOriginals.getFields.call(this, config);
				...
				for (var i2 in item.items) {
					...
					if (tab.id == "minishop2-product-tab" && tab.items[0]) {
						tab.items[0].items.push({
							title: _('msoptionscolor_color'),
							hideMode: 'offsets',
							items: [{xtype: 'msoptionscolor-panel-resource-inject'}]
						});
					}
				}
			}
	...
	msoptionscolorSetOptionValues: function (optionValues) {
